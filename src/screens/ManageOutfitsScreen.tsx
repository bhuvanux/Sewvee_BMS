...
const ManageOutfitsScreen = ({ navigation }: any) => {
    ...
    const pickImage = async () => {
        try {
            ...
            if (!result.canceled && result.assets[0].uri) {
                // PREVIEW STRATEGY: Use the original image directly for immediate feedback.
                // We will compress/convert it only when saving.
    ...
    const handleSave = async () => {
        ...
        if (editImage && (editImage.startsWith('file:') || editImage.startsWith('content:'))) {
           ...
           try {
                    // Process Image if custom new image selected
                    ...
                    if (editImage && editImage.startsWith('file://')) {
                        try {
                            ...
                            console.error("Failed to process image on save:", imgError);
